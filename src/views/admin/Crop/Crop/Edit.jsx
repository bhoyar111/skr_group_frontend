...
export default function Edit(props) {
    ...
    const getEditData = async () => {
        try {
            const getResponse = await AuthApi.get(`/admin/crop-get/${id}`);
            const { status, data } = getResponse;
            if( status === 200 && data.crop !== undefined ){
                const { crop } = data;
                for (let [key] of Object.entries(formInputObj)) {
                    formInputObj[key] = crop[key];
                }
    ...
    const submit = async () => {
        try {
            ...
            const editResponse = await AuthApi.put(`/admin/crop-update/${id}`, formData);
            const { status, data } = editResponse;
            if( status === 201 && data.crop !== undefined ){
                toast.success(`Crop Type edit successfully`);
